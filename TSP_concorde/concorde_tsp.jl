...
function concorde_st(data::DataCluCVRP, k::Int64, s::Int64, t::Int64, get_path::Bool)
    ...
    ncount = cluster_size(data,k) # Number of nodes (cities)
    ...
    for i in 0:ncount-1 # Concorde considers nodes ids starting at 0
        for j in i+1:ncount-1
            if i != j
                ...
                if (nodes[i+1] == s && nodes[j+1] == t) || (nodes[j+1] == s && nodes[i+1] == t)
                    elen[edgeWeight] = 0.0 # Endpoints cost
                end
    ...
    # Solve TSP
    appfolder = dirname(@__FILE__)
    open("$(appfolder)/../concorde.log", "w") do fileout
    ...
    for (i,v) in enumerate(out_tour)
        ...
        if nodes[v+1] == s pos_s = i  
        elseif nodes[v+1] == t pos_t = i end
    end

    # Check if edge (s,t) was used
    if (pos_s != pos_t-1 && pos_s != pos_t+1) && (pos_s != 1 && pos_t != ncount) && (pos_s != ncount && pos_t != 1)
        # @show nodes, aux, s, t
        error("Invalid Hamiltonian path, edge ($(aux[pos_s]),$(aux[pos_t])) (k=$k) not used!")
    end
